...
namespace SOS2HS
{
    public class StatWorker_CurrentCoolingPerSecond : StatWorker
    {
        ...
        public override float GetValueUnfinalized(StatRequest req, bool applyPostProcess = true)
        {
            if (req.Thing == null)
            {
                Log.Error(string.Concat("Getting CurrentCoolingPerSecond stat for ", req.Def, " without concrete thing. This always returns 1."));
                return 1;
            }
            return HeatsinkStatistics.GetCurrentCoolingPerSecond(req, applyPostProcess);
        }
        ...
        public override string GetExplanationUnfinalized(StatRequest req, ToStringNumberSense numberSense)
        {
            ...
            stringBuilder.AppendLine("StatsReport_SOS2HS_TargetTempDiff".Translate());
            stringBuilder.AppendLine("  (= " + "StatsReport_SOS2HS_TargetTempDiffEquation".Translate() + " )");
            stringBuilder.AppendLine("  (= " + "StatsReport_SOS2HS_TargetTempDiffEquationNumbers".Translate(targetTemp, cooledRoomTemp) + " )");
            stringBuilder.AppendLine("  " + targetTempDiff + " C");
            ...
            float maxCooling = energyPerSecond * efficiency / roomSurface * coolingConversionRate; // max cooling power possible
            stringBuilder.AppendLine("StatsReport_SOS2HS_MaxCooling".Translate());
            stringBuilder.AppendLine("  (= " + "StatsReport_SOS2HS_MaxCoolingEquation".Translate() + " )");
            stringBuilder.AppendLine("  (= " + "StatsReport_SOS2HS_MaxCoolingEquationNumbers".Translate(energyPerSecond, efficiency, roomSurface, coolingConversionRate) + " )");
            stringBuilder.AppendLine("  " + maxCooling + " C");

            float actualCooling = Mathf.Min(Mathf.Max(targetTempDiff, maxCooling), 0) * 60;
            stringBuilder.AppendLine("StatsReport_SOS2HS_ActualCoolingPerSecond".Translate());
            stringBuilder.AppendLine("  (= " + "StatsReport_SOS2HS_ActualCoolingPerSecondEquation".Translate() + " )");
            stringBuilder.AppendLine("  (= " + "StatsReport_SOS2HS_ActualCoolingPerSecondEquationNumbers".Translate(targetTempDiff, maxCooling) + " )");
            stringBuilder.AppendLine("  " + actualCooling + " C.s^-1");
        ...
        {
            return string.Format("{0} C", HeatsinkStatistics.GetCurrentCoolingPerSecond(optionalReq).ToString("0.###"));
        }
